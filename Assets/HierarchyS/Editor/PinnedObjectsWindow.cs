...
namespace Shadowprofile.HierarchyS
{
    public class PinnedObjectsWindow : EditorWindow
    {
        ...
        public static void LoadDataAsset()
        {
            if (pinnedData == null)
            {
                pinnedData = ShadowProfileUtils.GetAssetFromRelativePath<PinnedObjectsData>("HierarchyS/Editor", "PinnedObjectsData");
                Debug.Log($"PinnedData {pinnedData}");
                if (pinnedData == null)
                {
                    Debug.Log("Created New Asset");
                    pinnedData = CreateInstance<PinnedObjectsData>();
                    AssetDatabase.CreateAsset(pinnedData, $"{ShadowProfileUtils.GetFullPathFromRelativePath("HierarchyS/Editor")}/PinnedObjectsData.asset");
        ...
        public static class PinnedObjectsInjector
        {
            ...
            private static void AddPinnedObjectsButton(EditorWindow window)
            {
                ...
                Texture2D icon = ShadowProfileUtils.GetAssetFromRelativePath<Texture2D>("HierarchyS/Editor/Ico", "pinned");
