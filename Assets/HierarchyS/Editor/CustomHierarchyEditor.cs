...
namespace Shadowprofile.HierarchyS
{
        ...
        public static readonly Dictionary<string, string> s_IconMap = new()
    {
        ...
        { "DecalProjector", "DecalProjector Icon" },
        ...
        private static CustomIconData s_CustomIconData
        {
            get
            {
                if (i_CustomIconData == null) i_CustomIconData = ShadowProfileUtils.GetAssetFromRelativePath<CustomIconData>("HierarchyS/Editor", "CustomIconData");
        ...
        private static Texture2D s_WarningIcon
        {
            get
            {
                if (i_WarningIcon == null) i_WarningIcon = EditorGUIUtility.IconContent("console.warnicon").image as Texture2D;
        ...
        private static Texture2D s_ErrorIcon
        {
            get
            {
                if (i_ErrorIcon == null) i_ErrorIcon = EditorGUIUtility.IconContent("console.erroricon").image as Texture2D;
                return i_ErrorIcon;
            }
        }
        ...
        private static Texture2D s_pinIcon
        {
            get
            {
                if (i_pinIcon == null) i_pinIcon = (Texture2D)ShadowProfileUtils.GetAssetFromRelativePath<Texture2D>("HierarchyS/Editor/Ico", "pin");
                return i_pinIcon;
            }
        }
        ...
        private static Texture2D GetComponentIcon(Type componentType, List<Component> components)
        {
        ...
        private static void OpenInspector(Component[] components)
        {
        ...
        private static bool HasColliderNegative(GameObject gameObject)
        {
            foreach (var collider in gameObject.GetComponents<Collider>())
            {
                switch (collider)
                {
                    ...
                    case CapsuleCollider capsuleCollider when (capsuleCollider.radius <= 0 || capsuleCollider.height <= 0):
                        return true;
                }
            }
